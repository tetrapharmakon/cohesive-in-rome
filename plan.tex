...
\begin{frame}{Toposes}
	...
	\begin{itemize}
		...
		\item<4-> Differential geometry
		\item<5-> (Algebraic topology)
		\item<6-> \dots
	\end{itemize}
\end{frame}
%
...
%
%
\begin{frame}[label={sec:org4e45118}]{Grothendieck topologies}
...
%
%
\begin{frame}[label={sec:org9262f53}]{Sheaves on a site}
	\begin{block}{}
	...
	\onslide<2->
	(by the Yoneda lemma, this consists of a unique element $s\in FU$, \textbf{exercise}).
...
%
%
\begin{frame}[label={sec:org5bf1529}]{Giraud Theorem}
...
%
%
\begin{frame}[label={sec:org6a1b958}]{Elementary toposes}
	\begin{block}{}
...
\begin{frame}{Elementary toposes}
	The natural bijection $\clE(A,\Omega)\cong\text{Sub}(A)$ is obtained pulling back a ``characteristic arrow'' $\chi_U : A \to \Omega$ along a \emph{universal arrow} $t : 1\to \Omega$ to obtain the monic $U$, as in the diagram
	\[
...
%
%
\begin{frame}[label={sec:orgb2a88e9}]{Grothendieck $\subset$ elementary}
...
\begin{itemize}
	\item<+-> Every category $\clC$ is a universe in which we can interpret type theory;
	\item<+-> every object $A\in \clC$ is a type --a term in the type of types:
	\item<+-> every morphism $X \to A$ is a (generalised) term of type $A$, in a context $X$.
\end{itemize}
